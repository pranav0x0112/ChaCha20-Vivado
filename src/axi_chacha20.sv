...
module axi_chacha20 #(
    ...
    // Internal registers
    // =========================================================================
    logic [31:0] reg_key    [0:7];   // 0x08-0x24
    logic [31:0] reg_nonce  [0:2];   // 0x28-0x30
    logic [31:0] reg_counter;         // 0x34
    logic [31:0] reg_keystream [0:15]; // 0x38-0x74 (latched from core)

    logic        core_start;          // 1-cycle pulse to chacha20_top
    logic        core_done;
    ...
    logic aw_active; // write address accepted
    logic w_active;  // write data accepted
    ...
    // Latch keystream words when core signals done
    ...
    always_ff @(posedge S_AXI_ACLK) begin
        if (!S_AXI_ARESETN) begin
            ...
            if (axi_wready && S_AXI_WVALID && aw_active) begin
                case (axi_awaddr[7:2]) // word-addressed (drop bottom 2 bits)
